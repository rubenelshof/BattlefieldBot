...
bot.on("message", function(message) {
  if (!message.channel.isPrivate) {
    ...
    //api
    var bfbc2statsapi = "http://api.bfbcs.com/api/pc?players=" + playername + "&fields=general&callback=?";
...
bot.on("message", function(message) {
  if (!message.channel.isPrivate) {
    ...
    //api
    var bf3statsapi = "http://api.bf3stats.com/pc/player/player=" + playername + "&opt=clear,rank,global&callback=?";
...
bot.on("message", function(message) {
  if (!message.channel.isPrivate) {
    ...
    //api
    var bf4statsapi = "http://api.bf4stats.com/api/playerInfo?plat=pc&name=" + playername + "&opt=stats,extra&callback=?";
...
bot.on("message", function(message) {
  if (!message.channel.isPrivate) {
    ...
    if (message.content === ("!bf1rank " + playername)) {getJSON(bf1statsapi, function(err, res, callback) {bot.sendMessage(message, "Your rank is **" + res.stats.rank + "**");});}
    if (message.content === ("!bf1skill " + playername)) {getJSON(bf1statsapi, function(err, res, callback) {bot.sendMessage(message, "Your skill score is **" + res.stats.skill + "**");});}
    if (message.content === ("!bf1kd " + playername)) {getJSON(bf1statsapi, function(err, res, callback) {bot.sendMessage(message, "Your K/D is **" + res.stats.extra.kdr.toFixed(2) + "**");});}
  } else {
    if (message.content === "!stats") {
      bot.sendMessage(message, "Hello, Your on **PM**");
    }
